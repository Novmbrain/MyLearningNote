...
![image-20210108162240340](D:\编程学习\MD笔记\尚硅谷_宋红康_JDBC.assets\JavaWeb技术概览.png)
...
- PreparedStatement 可以防止 SQL 注入。因为预编译已经确定了SQL的逻辑，剩下的值通过占位符输入。 这样避免直接传入恶意SQL语句                                                                                
...
public void testInsert() {
    ...
    try {
        ...
        conn = DriverManager.getConnection(url, user, password);
        System.out.println(conn);
        //4.预编译 sql语句，返回PreparedStatement语句
...
// 通用的针对于不同表的查询:返回一个对象 (version 1.0)
	public <T> T getInstance(Class<T> clazz, String sql, Object... args) {
		...
		try {
			...
			int columnCount = rsmd.getColumnCount();
			if (rs.next()) {//next()有两个功能，一个是判断下个位子是否有元素，若没有元素则进指针下移
				...
				for (int i = 0; i < columnCount; i++) {// 遍历每一个列
					...
					// 获取列的别名:列的别名，使用类的属性名充当
                     //因为如果直接获取列名，然后通过反射用列名去类中寻找对应的属性名，很可能因为命名规则不一致						而找不到。所以我们听过给字段起别名，使用别名和属性名一致的方式规避和这个问题
...
- ResultSetMetaData meta = rs.getMetaData();
  ...
  - **getColumnLabel**(int column)：获取指定列的别名，如果没有起别名则返回列名。所以其实可以用getColumnLabel直接替换getColumnName
...
- birth字段在数据库中是Date类型，在对占位符？进行替换时，可以直接传入符合格式的字符串，数据库会自动进行隐式转换。
...
rs = ps.executeQuery();
if(rs.next()){
//这是赋值方式一，使用的是角标的方式，这样角标顺序与SQL语言查询顺序要对应
//	Integer id = rs.getInt(1);
//  String name = rs.getString(2);
//	String email = rs.getString(3);
//    Date birth = rs.getDate(4);

    //方式二：这种方式不依赖SQL语言的查询字段顺序
    ...
    public void test1(){
        ...
        try {
            ...
            if(rs.next()) {
                ...
                fileOutputStream = new FileOutputStream("c.jpg");
                byte[] buffer = new byte[1024];
                int len = 0;
 ...
 * 		 ?rewriteBatchedStatements=true 写在配置文件的url后面
 		MySQL8以上，用&&rewriteBatchedStatements=true
...
public abstract class BaseDao<T> {
	...
	public BaseDao() {
		// 获取子类的类型
        /**
        在类的构造器中，我们可以使用"this.属性"或"this.方法"的方式，调用当前正在创建的对象属性或方法。但是，通		 常情况下，我们都择省略"this."
       这里将BaseDao的泛型获取的代码块写在了BookDaoImpl的父类BaseDao中，但是因为实际上创建的类是			   BookDaoImpl，this.方法调用当前正在创建的对象属性或方法。所以在代码块执行时，this指的是BookDaoImpl
        */
	...
	/**
	 * 获取一个唯一值的方法，专门用来执行像 select count(*)...这样的sql语句
	 * 
	 * @param sql
	 * @param params
	 * @return
...
<c3p0-config>
	<named-config name="helloc3p0">
		...
		<property name="password">1243</property>
        <!-- 如果连接的是本地端口可以省略  localhost:3306 
...
public void testQueryInstance1() throws Exception{
	...
	ResultSetHandler<Customer> handler = new ResultSetHandler<Customer>() {
		//重写的handle方法的返回值Customer就作为之后runner.query的返回值
		@Override
		public Customer handle(ResultSet rs) throws SQLException {//query查询得到的结果集作为参数ResultSet传入这个重写方法，然后在方法中对ResultSet进行处理。
